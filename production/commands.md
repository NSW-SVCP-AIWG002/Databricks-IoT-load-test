...
> Master のみ docker-compose v1。Worker VM 1-8 はすべて docker compose v2 で統一。
...
```bash
cd ~/iotbricks/production && docker rm -f locust-master 2>/dev/null; docker ps -a | awk '{print $1}' | tail -n +2 | xargs docker rm -f 2>/dev/null; > output/locust.log; docker-compose up -d master
```
