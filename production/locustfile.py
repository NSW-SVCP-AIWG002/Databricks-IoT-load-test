...
_connected_count = 0
_failed_count = 0
_issued_count = 0   # credential を取得したユーザー数（実際のスポーン数と一致）
_spawning_complete = False              # 全ユーザースポーン完了フラグ
    ...
    def on_start(self):
        ...
        if connection_error is not None:
            ...
            with _connected_count_lock:
                ...
                if _spawning_complete and total_done >= _issued_count:
...
def on_spawning_complete(user_count, **kwargs):
    global _spawning_complete
    _spawning_complete = True
    print(f"[スポーン完了] {user_count}ユーザー起動完了")
    # 全員がすでに接続完了/失敗していた場合に備えてここでも判定
    with _connected_count_lock:
        total_done = _connected_count + _failed_count
